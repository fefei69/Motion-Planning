...
class Environment(object):
  ...
  def collision_check(self, path)->bool:
    _, is_collision= collision_checking(self.blocks, path, self.block_planes, ax=None, verbose=False)
    return is_collision
  @staticmethod
...
def main():
  # start, goal, mapfile = test_single_cube() # 0.8s Path length: 8.24 (resolution 0.1), 8.47(resolution 0.3)
  start, goal, mapfile = test_maze() #51:24 Path length: 75.4010 resolution 0.3; 21:09 Path length: 80.89711 resolution 0.4
  # start, goal, mapfile = test_flappy_bird() # 04:37 path length: 26.25 (resolution 0.3)
  # start, goal, mapfile = test_monza() # 02:18 Path length: 76.38 (resolution 0.3)
  # start, goal, mapfile = test_window() # 06:51 Path length: 26.92478 (resolution 0.3)
  # start, goal, mapfile = test_tower() # 11:57 Path length: 26.4953 (resolution 0.3)
  # start, goal, mapfile = test_room() #01:15 Path length: 11.39 (resolution 0.3)
  ...
  open_nodes = [node.position for node in open.values()]
  open_nodes = np.array(open_nodes)
  ax.scatter(open_nodes[:,0], open_nodes[:,1], open_nodes[:,2], s=1, marker='s', color='grey',alpha=0.05)
  closed_nodes = [node.position for node in closed.values()]
  closed_nodes = np.array(closed_nodes)
  ax.scatter(closed_nodes[:,0], closed_nodes[:,1], closed_nodes[:,2], marker='.', color='orange',alpha=0.05)
